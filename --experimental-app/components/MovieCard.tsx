...
const MovieCard: React.FC<MovieCardProps> = ({ movie, isLarge = false, orientation = 'portrait' }) => {
  ...
  return (
    <div className={`flex-none relative transition-transform duration-300 hover:scale-105 hover:z-10 cursor-pointer ${widthClass}`}>
      <Link href={`/movie?id=${movie.id}`} className="block">
